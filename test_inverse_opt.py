...
def test4(fvalue=False, lasso=False):
    ...
    for i in [0.00001, 0.0001, 0.001]:
        for j in [0.0001, 0.001, 0.01]:
            for k in [0.00001, 0.0001, 0.001]:
                smooth = np.hstack((i*np.ones(degree/3), j*np.ones(degree/3), k*np.ones(degree/3)))
                if fvalue:
                    theta, f = invopt.solver([graph1, graph2, graph3], [l1, l2, l3], degree, smooth, fvalue=True, lasso=lasso)
                else:
                    theta = invopt.solver([graph1, graph2, graph3], [l1, l2, l3], degree, smooth, lasso=lasso)
                ...
                if error < min_error:
                    best_smooth, min_error, best_theta = (i,j,k), error, theta
                    if fvalue: obj = f
    ...
    plt.xlabel('Link flow')
    plt.ylabel('Delay')
    plt.title(r'Estimated delay function. l2-norm error: {:.3f}'.format(min_error))
    plt.legend()
    plt.show()
        ...
        for j in [600.0, 1000.0, 3000.0]:
            ...
            if fvalue:
                theta, f1, f2 = invopt.solver_mis([graph1, graph2, graph3], [l1[obs], l2[obs], l3[obs]], 
                                  indlinks_obs, degree, smooth, None, max_iter, fvalue=True)
            else:
