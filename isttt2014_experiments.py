...
def synthetic_data(data=None, SO=False, demand=3, N=4):
    ...
    if SO: l = ue.solver(g, update=True, SO=SO)
    else: l = ue.solver(g, update=True)
    n = g.numlinks
    g.visualize()
    ...
    obs[N-1] = range(n)
    return g, f_true, l, wp_trajs, obs
...
def experiment(data=None, SO=False, trials=10, demand=3, N=10, plot=False, withODs=False, data_id=None):
    ...
    for k in range(trials):
        g, f_true, l, wp_trajs, obs = synthetic_data(data, SO, demand, N)
